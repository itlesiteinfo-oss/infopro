...
- **Design « Découvrir »** (`mobile_layout = card`, `mobile_card_thumb` 80–220 px) : `Renderer::card_metrics()` donne interligne, image (largeur × 0,625), hauteur `2 × 14 + 26 + 10 + max(lignes × interligne, hauteur image)` et bandeau replié `14 + 26 + 2`. Le lien passe en flex pour qu'un article sans image récupère toute la largeur, gouttière comprise ; `Settings::wants_thumbnails()` inclut ce design pour que l'`<img>` entre dans le markup mis en cache. Le style de label `strip` y devient un titre en gras sans fond.
