...
- **Design « Découvrir »** (`mobile_layout = card`, `mobile_card_thumb` 80–220 px) : `Renderer::card_metrics()` donne la police du titre (profil + 2 px), l'interligne (× 1,5), l'image (largeur × 0,625), le nombre de lignes `max(réglage, ⌊hauteur image / interligne⌋)` (exposé en `--hprnb-m-card-lines`), la hauteur `2 × 14 + 26 + 10 + max(lignes × interligne, hauteur image)` et le bandeau replié `14 + 26 + 2`. L'image ouvre la ligne (`order: 0`, donc à droite en RTL) sous la ligne de titre ; le titre suit en flex, un article sans image récupérant toute la largeur, gouttière comprise. Les boutons sont placés un à un (`.hprnb-bar__controls { display: contents }`) : le bouton Fermer devient un onglet carré de 52 × 46 px au-dessus du coin de fin (`inset-block: auto 100%`, `inset-inline-end: 0`, couleur de la barre, liseré d'accent), la barre et l'inner passant en `overflow-x: clip; overflow-y: visible` pour le laisser sortir ; Pause et le chevron restent dans la ligne de titre. Le design ignore `mobile_controls_place` / `mobile_controls_layout` (classes non émises) et `mobile_controls()` y renvoie 0. `Settings::wants_thumbnails()` inclut ce design pour que l'`<img>` entre dans le markup mis en cache ; le style de label `strip` y devient un titre en gras sans fond.
