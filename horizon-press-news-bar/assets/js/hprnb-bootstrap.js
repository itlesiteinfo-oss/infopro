	...
	function run() {
		...
		function apply(p) {
			...
			if (old && old.getClientRects().length) {
				clock = 5000;
				(old.getAnimations ? old.getAnimations() : []).forEach(function (a) {
					if (a.animationName === 'hprnb-u-bn-open') {
						clock = (a.effect.getComputedTiming().localTime || 0) - (a.effect.getTiming().delay || 0);
					}
				});
			}
			var wait = {};
