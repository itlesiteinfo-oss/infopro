...
*Minutage de la 2.18 ; la 2.19 l'a resserré (voir 9 tervicies).* Section 17 quater de `hprnb-bar.css`, sous `@media (prefers-reduced-motion: no-preference)` et la classe `hprnb-root--u-bn` : `hprnb-u-bn-open` (le bandeau découvert par `clip-path`, 0–450 ms), `hprnb-u-bn-in` (le cartouche, 24 px et fondu, 280–580 ms), `hprnb-u-bn-rule` (le filet, `scaleX` depuis le cartouche sous 600 px, `scaleY` depuis son milieu au-delà, 430–760 ms), `hprnb-u-bn-ctrl` (les boutons, 620–800 ms). L'animation du bandeau dure toute l'ouverture (860 ms) et sert d'horloge : le moteur (`openingLeft()`) fait partir le premier titre à sa fin, le script d'amorçage hybride la reporte (`--hprnb-u-bn-t`, délai négatif) sur un bandeau qui en remplace un en cours d'ouverture. Une fois ouvert, le bandeau est marqué `data-hprnb-opened` et ne se rouvre plus (masqué puis réaffiché, préférence changée, rotation, redimensionnement) ; seul un bandeau nouveau s'ouvre. Sens de lecture : variables `--hprnb-u-bn-start`, `--hprnb-u-bn-open-0`, `--hprnb-u-bn-in` inversées sous `:dir(rtl)` / `.hprnb-bar--rtl`. Aperçu : `replayPreview()` et le bouton `#hprnb-preview-replay`.

## 9 tervicies. La colonne « Urgent » de la liste des articles, l'ouverture resserrée (2.19)

**Liste des articles** (`Admin\Urgent_List`, seulement quand le bandeau URGENT est activé sur un appareil) : colonne `hprnb_urgent` insérée juste après `title` (`manage_post_posts_columns` ; une colonne tierce comme « VAR ARTICLE » n'est pas touchée), cellule rendue par `Urgent_List::cell()` — un `<button type="button" class="hprnb-urgent-switch" aria-pressed>` à l'état `data-hprnb-state` `off` / `armed` / `active` (`Urgent::state()` : « actif » seulement publié avec son compte à rebours en cours), un badge `is-static` (état en texte pour les lecteurs d'écran) pour qui ne peut pas modifier l'article ou qui l'a à la corbeille ; « en cours de modification » quand quelqu'un d'autre l'a ouvert (verrou d'édition), l'interrupteur restant utilisable. La case de l'écran d'édition envoie ce que l'article portait à son affichage (`hprnb_urgent_seen`) et si elle a été touchée (`hprnb_urgent_touched`) : non touchée sur un article changé entre-temps, elle est ignorée. Un article qui quitte « publié » pendant son compte à rebours est remis en attente de publication (`arm()`), ou décoché s'il part à la corbeille. Ligne `hprnb-urgent-row` (via `post_class`, y compris la ligne rendue par la Modification rapide) : teinte à 6 % et liseré `inset 4px` du rouge `urgent_bg_color`. Vue `edit.php?hprnb_urgent=1` : `views_edit-post` (compte), `pre_get_posts` (`post__in` des identifiants cochés, par deux requêtes d'identifiants sur une clé de méta chacune), `restrict_manage_posts` (champ caché : recherche et filtres gardent la vue).

**Route** `hprnb/v1/urgent/<id>` : `POST { urgent: bool }` (nonce `wp_rest`, `edit_post`, bandeau activé) écrit par `Urgent::apply()` — la règle de la case de l'écran d'édition, sans relancer un compte à rebours en cours — et répond `{ id, state, until, html, count }` ; `GET` renvoie la même chose (`edit_posts` et `read_post`). Script `hprnb-urgent-list.js` (dépendance `wp-a11y`) : attente sans double requête, refus expliqué affiché, toute autre erreur relue par `GET`, nonce expiré renouvelé par `admin-ajax.php?action=rest-nonce`, ligne relue à la fin du compte à rebours (`data-hprnb-left`). Styles `hprnb-urgent-list.css`.

**Ouverture** (section 17 quater) : 0–420 ms pour le rouge depuis la 2.19.1 (57,53 % d'une animation de 730 ms ; 0–320 ms sur 700 ms en 2.19.0), 190–440 ms pour le cartouche, 300–580 ms pour le filet (`ease-in-out` : aux trois quarts quand le bouton fermer arrive), 480–650 ms pour les boutons (échelle 0,92), titre à 730 ms, 80 ms après les boutons (`BN.open`), le script attendant la première image réelle de l'animation (`openingPending()`). **Frappe** : une seule surbrillance `hprnb-u-bn-rest` (ou `span.hprnb-bar__rest`) transparente sur la partie pas encore tapée ; chaque lettre tapée est directement dans la couleur du titre.
