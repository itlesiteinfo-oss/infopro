...
**Ouverture** (section 17 quater) : 0–320 ms pour le rouge (45,71 % d'une animation de 700 ms), 190–440 ms pour le cartouche, 300–580 ms pour le filet (`ease-in-out` : aux trois quarts quand le bouton fermer arrive), 480–650 ms pour les boutons (échelle 0,92), titre à 700 ms (`BN.open`), le script attendant la première image réelle de l'animation (`openingPending()`). **Frappe** : une seule surbrillance `hprnb-u-bn-rest` (ou `span.hprnb-bar__rest`) transparente sur la partie pas encore tapée ; chaque lettre tapée est directement dans la couleur du titre.
